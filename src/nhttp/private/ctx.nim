...
type
  ...
  VhostTable* = CritBitTree[HttpHandler]
  ...
  ServerCtx* = ref object
    ...
    timeout*: int
    poolsize*: int
    handler*, hookRequest*, hookResponse*: HttpHandler
    vhostTbl*: VhostTable
    isSSL*: bool
    when defined ssl:
...
proc newServerCtx*(servername = "mofuw", port: int,
                  handler,
                  hookrequest, hookresponse: HttpHandler = nil,
                  readBufferSize, writeBufferSize = 4096,
                  maxBodySize = 1024 * 1024 * 5,
                  timeout = -1,
                  poolsize = 128,
                  isSSL = false): ServerCtx =
  result = ServerCtx(
